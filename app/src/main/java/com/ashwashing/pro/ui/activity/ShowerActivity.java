...
import com.ashwashing.pro.impl.analyze.CMDUtils;
import com.ashwashing.pro.impl.analyze.WaterCodeListener;
 ...
 * </P>
 */
public class ShowerActivity extends BaseActivity implements BluetoothService.Callback, WaterCodeListener {
    ...
    public void onConnected() {
        CMDUtils.chaxueshebei(mBluetoothService, false);
    }
    ...
    @Override
    public void caijishujuOnback(boolean success, final String consumeDT, final int productID, final int deviceID, final int accountID, String groupID, final int useCount, String perMoney, String usedMoney, String rate, String mac, byte[] raw) {
        if (success) {
            ...
            mSprayService.end(ApiUtils.generateHeaderMap(), mDevice.getAddress(), formattedConsumeDT, usedMoney).enqueue(new ShowerSimpleResultCallbackAdapter<SimpleResult>(ShowerState.END_SPRAY_ERROR) {
                ...
                public void onSuccess(SimpleResult result) {
                    CMDUtils.fanhuicunchu(mBluetoothService, true, consumeDT, productID, deviceID, accountID, useCount);
                }
            });
        } else {
    ...
    @Override
    public void chaxueNewshebeiOnback(boolean success, int deviceStatus, int deviceID, int productID, int accountID, final byte[] mac, final byte[] tac, int i5, int i6, int chargeMethod, int i8, String str) {
        if (success) {
            switch (deviceStatus) {
                case 0:
                    if (mDeviceCheckedUsable) {
                        mSprayService.start(ApiUtils.generateHeaderMap(), mDevice.getAddress()).enqueue(new ShowerDataResultCallbackAdapter<DataResult<SprayInfo>>(ShowerState.START_SPRAY_ERROR) {
                            ...
                            public void onSuccess(DataResult<SprayInfo> result) {
                                ...
                                CMDUtils.dispatchFeeRate(mBluetoothService, mac, tac, info.getCommand());
                            }
                        });
                    } else {
                ...
                case 1:
                    if (accountID != mUserInfo.AccID && mDeviceCheckedUsable) {
                        ...
                        mCheckService.check(ApiUtils.generateHeaderMap(), mDevice.getAddress(), accountID, productID, deviceID).enqueue(new ShowerSimpleResultCallbackAdapter<SimpleResult>(ShowerState.CHECK_CLIENT_ERROR) {
                            @Override
                            public void onSuccess(SimpleResult result) {
                                UiUtils.createAlert(ShowerActivity.this, R.string.continue_shower_or_end)
                            ...
                            public void onError(int code, String msg) {
                                if (code == Constants.CODE_EX_CLIENT) {
                                    onNotSuccess();
                                    UiUtils.createAlert(ShowerActivity.this, R.string.use_ex_client)
                ...
                case 2:
                    ...
                    break;
                case 3:
                    CMDUtils.caijishuju(mBluetoothService, true);
                    break;
            }
        } else {
    ...
    @Override
    public void fanhuicunchuOnback(boolean success) {
        if (success) {
            //the spray now stopped and the shower is finished
            setShowerState(ShowerState.FINISHED, false);
            mEndTimestamp = System.currentTimeMillis();
    ...
    @Override
    public void jieshufeilvOnback(boolean success, int accountID) {
        if (success) {
            CMDUtils.caijishuju(this.mBluetoothService, true);
        } else {
            CMDUtils.chaxueshebei(this.mBluetoothService, true);
        }
    }
    ...
    @Override
    public void xiafafeilvOnback(boolean success) {
    ...
    public void progressShowerState(View view) {
        switch (mState) {
            case PENDING:
                switchToLoadingState(true);
                CMDUtils.chaxueshebei(mBluetoothService, true);
                break;
            case SHOWERING:
                switchToLoadingState(true);
                CMDUtils.jieshufeilv(mBluetoothService, true);
                break;
            case INTERRUPTED:
                mBluetoothService.connect();
                break;
    ...
    private class InfoAdapter extends RecyclerView.Adapter<InfoAdapter.InfoViewHolder> {
        ...
        public InfoViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View item = LayoutInflater.from(ShowerActivity.this).inflate(R.layout.item_info, parent, false);
